...
class CustomSurvey(Survey):
    ...
    def survey_display_page(self, survey_token, answer_token, **post):
        ...
        print(post, answer_sudo, self._prepare_survey_data(
    ...
    def survey_used_in_computation(self, **kwargs):
        ...
        for question in question_id:
            print(question.calculated_metric_ids)
            if question.calculated_metric_ids.operand_1 or question.calculated_metric_ids.operand_2:
                metrics = request.env['metric.management'].sudo().search([('calculated_metric_id', '=', question.calculated_metric_ids.operand_1.id)])
                metrics2 = request.env['metric.management'].sudo().search([('calculated_metric_id', '=', question.calculated_metric_ids.operand_2.id)])
                question_1 = metrics.sudo().mapped('survey_question_id').filtered(lambda x:x.survey_id.id == question.survey_id.id)
                question_2 = metrics2.sudo().mapped('survey_question_id').filtered(lambda x:x.survey_id.id == question.survey_id.id)
                print(question_1, question_2, "metrics")
        return [{'id': question_1.sudo().id,
                 'type': question_1.sudo().question_type},
                {'id': question_2.sudo().id,
                 'type': question_2.sudo().question_type}]
    ...
    def survey_calculated_metric_info_answer(self, **kwargs):
        if kwargs.get('question_id'):
            ...
            print(survey_question.survey_id.question_and_page_ids.filtered(
                lambda x:x.question_type == 'calculated_metric').calculated_metric_ids, "survey_question")
            answers = {}
            for question in survey_question.survey_id.question_and_page_ids.filtered(
                    lambda x: x.question_type == 'calculated_metric'):
                print(question.calculated_metric_ids)
                if question.calculated_metric_ids.operand_1 or question.calculated_metric_ids.operand_2:
                    metrics = request.env['metric.management'].sudo().search([(
                                                                              'calculated_metric_id',
                                                                              '=',
                                                                              question.calculated_metric_ids.operand_1.id)])
                    metrics2 = request.env['metric.management'].sudo().search([(
                                                                               'calculated_metric_id',
                                                                               '=',
                                                                               question.calculated_metric_ids.operand_2.id)])
                    question_1 = metrics.mapped('survey_question_id').filtered(
                        lambda x: x.survey_id.id == question.survey_id.id)
                    question_2 = metrics2.mapped('survey_question_id').filtered(
                        lambda x: x.survey_id.id == question.survey_id.id)
                    print(question_1, question_2, "metrics")
            # for question in survey_question.survey_id.question_and_page_ids.filtered(
            #     lambda x: x.question_type == 'calculated_metric'):
        ...
        calculated_metrics = request.env['survey.question'].sudo().search(['|',
                                                                           ('id', '=', question_1.id),
                                                                           ('id', '=', question_2.id)])
        print(calculated_metrics, "survey_calculated_metric_info_answer")
        for calculated_metric in survey_question.survey_id.question_and_page_ids.filtered(
                lambda x: x.question_type == 'calculated_metric'):

            dict_add = True
            dict = {}
            print(calculated_metric, dict_add, "survey_calculated_met")
            ...
            if dict_add:
                print(dict_add, "dict")
                if calculated_metric.calculated_metric_ids.operand_1 or calculated_metric.calculated_metric_ids.operand_2:
                    metrics = request.env['metric.management'].sudo().search([(
                                                                              'calculated_metric_id',
                                                                              '=',
                                                                              calculated_metric.calculated_metric_ids.operand_1.id)])
                    metrics2 = request.env['metric.management'].sudo().search([(
                                                                               'calculated_metric_id',
                                                                               '=',
                                                                               calculated_metric.calculated_metric_ids.operand_2.id)])
                    question_1 = metrics.mapped('survey_question_id').filtered(
                        lambda x: x.survey_id.id == calculated_metric.survey_id.id)
                    question_2 = metrics2.mapped('survey_question_id').filtered(
                        lambda x: x.survey_id.id == calculated_metric.survey_id.id)
                    print(question_1, question_2, "metrics12345")
                if question_1.id == int(
                        kwargs.get('question_id')):
                    dict[
                        str(question_1.id)] = kwargs.get(
                        'answer')
                else:
                    dict[
                        str(question_1.id)] = ''
                if question_2.id == int(
                        kwargs.get('question_id')):
                    dict[
                        str(question_2.id)] = kwargs.get(
                        'answer')
                else:
                    dict[
                        str(question_2.id)] = ''
    ...
    def survey_calculated_metric_info2(self, kwargs):
        ...
        if question_id.calculated_metric_ids.operand_1 or question_id.calculated_metric_ids.operand_2:
            metrics = request.env['metric.management'].sudo().search([(
                'calculated_metric_id',
                '=',
                question_id.calculated_metric_ids.operand_1.id)])
            metrics2 = request.env['metric.management'].sudo().search([(
                'calculated_metric_id',
                '=',
                question_id.calculated_metric_ids.operand_2.id)])
            question_1 = metrics.mapped('survey_question_id').filtered(
                lambda x: x.survey_id.id == question_id.survey_id.id)
            question_2 = metrics2.mapped('survey_question_id').filtered(
                lambda x: x.survey_id.id == question_id.survey_id.id)
            operand1_question = question_1
            operand2_question = question_2
            operator = question_id.calculated_metric_ids.operator
            ...
            if operand2_question.question_type == 'numerical_box' or operand2_question.question_type == 'progress_bar' and \
                ...
                if kwargs.get(str(operand2_question.id)) != '':
                    ...
                    operand2_value = ''
            print(operand2_value, "operand_2")
            # Computation
