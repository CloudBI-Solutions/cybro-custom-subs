...
class BadmintooController(Survey):
    ...
    @route(['/my/badmintoo/assessment',
            '/my/badmintoo/assessment/page/<int:page>'], type='http',
    ...
    def my_badmintoo_assessment(self, search=None, page=0, **kw):
        ...
        assessment_plans = request.env['assessment.plan'].sudo().search([])
        total = len(assessments)
        pager = request.website.pager(
            url='/my/badmintoo/assessment/',
            total=total,
            page=page,
            step=6,
        )
        ...
        for rec in athletes:
            assessment = request.env['badminto.assessment'].sudo().create({
                ...
                'assessment_plan_id': int(
                    post.get('assessment_plans')) if post.get(
                    'assessment_plans') else None,
            })
    ...
    def my_assessment_details(self, **post):
        if post.get('assessment'):
            ...
            lifestyle_assessment_id = request.env[
                ...
                'config.lifestyle_data_survey_id')
            answers = False
            ...
            hrv_assessment_id = request.env[
                'ir.config_parameter'].sudo().get_param(
                'config.hrv_data_survey_id')
            hrv_answers = False
            if hrv_assessment_id:
                hrv_assessment = request.env['survey.survey'].sudo().browse(
                    int(hrv_assessment_id))
                hrv_answers = hrv_assessment.user_input_ids.filtered(lambda x:x.partner_id.id == request.env.user.partner_id.id and x.state == 'done')
            mobility_assessment_id = request.env[
                'ir.config_parameter'].sudo().get_param(
                'config.mobility_data_survey_id')
            mobility_answers = False
            if mobility_assessment_id:
            ...
            mental_assessment_id = request.env[
                'ir.config_parameter'].sudo().get_param(
                'config.mental_data_survey_id')
            mental_answers = False
            if mental_assessment_id:
            ...
            anaerobic_assessment_id = request.env[
                'ir.config_parameter'].sudo().get_param(
                'config.anaerobic_data_survey_id')
            anaerobic_answers = False
            if anaerobic_assessment_id:
            ...
            aerobic_assessment_id = request.env[
                'ir.config_parameter'].sudo().get_param(
                'config.aerobic_data_survey_id')
            aerobic_answers = False
            if aerobic_assessment_id:
            ...
            nutrition_assessment_id = request.env[
                'ir.config_parameter'].sudo().get_param(
                'config.nutrition_data_survey_id')
            nutrition_answers = False
            if nutrition_assessment_id:
            ...
            badminto_fields = attachments.mapped(
                lambda x: x.badminto_field_name)
            print('fields names', badminto_fields)
            print('assssss', assessment)
    ...
    def my_lifestyle_assessment(self, answer_token=False,email=False, **post):
        assessment = request.env['badminto.assessment'].sudo().browse(
            int(post.get('assessment')))
        lifestyle_assessment_id = request.env['ir.config_parameter'].sudo().get_param('config.lifestyle_data_survey_id')
        ...
        if lifestyle_assessment_id:
            ...
            if not answers:
                ...
                return request.render('badminto.lifestyle_assessment_visual_template', values)
            # else:
            #     for rec in answers:
