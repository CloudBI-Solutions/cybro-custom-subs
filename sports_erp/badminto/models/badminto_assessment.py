...
class BadmintoAssessment(models.Model):
    ...
    assessment_plan_id = fields.Many2one('assessment.plan',
    ...
    def create(self, vals):
        res = super(BadmintoAssessment, self).create(vals)
        assessment_types = self.env['assessment.types'].sudo().browse(
            res.assessment_type_ids.ids)
        athlete = res.athlete_id
        assessment_names = assessment_types.mapped(
            lambda x: x.name)
        name_assessment = []
        for rec in assessment_names:
            name_assessment.append([word[0] for word in rec.split()])
        real_name = ''
        for rec in name_assessment:
            name_string = ''
            for word in rec:
                name_string += word
            real_name = real_name + name_string + '/'
        name_name = real_name + athlete.partner_id.name + ' ' + athlete.partner_id.last_name + '/' + str(date.today()) + '/' + self.env['ir.sequence'].sudo().next_by_code(
            'badminto.assessment')
        res.name = name_name
        return res
    ...
    def upload_video_badminto(self, rec_id, upload_data):
        for rec in upload_data:
            attachments_present = self.env['ir.attachment'].search([
                ('res_model', '=', 'badminto.assessment'),
                ('res_id', '=', rec_id),
                ('badminto_field_name', '=', rec['field_name'])
            ])
            if attachments_present:
                attachments_present.unlink()
            attachment = self.env['ir.attachment'].create({
                'name': rec['file_name'],
                'badminto_video_file_data': rec['data'],
                'res_model': 'badminto.assessment',
                'res_id': rec_id,
                'badminto_field_name': rec['field_name'],
            })
    ...
    def update_video_badminto(self, rec_id, delete_datas):
        for rec in delete_datas:
            attachment = self.env['ir.attachment'].search([
                ('res_id', '=', rec_id),
                ('res_model', '=', 'badminto.assessment'),
                ('badminto_field_name', '=', rec)
                ])
            attachment.unlink()
