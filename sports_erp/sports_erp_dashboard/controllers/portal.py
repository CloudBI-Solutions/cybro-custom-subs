...
class Organisation(CustomerPortal):
    ...
    def my_coaches(self, page=0, search='', **post):
        ...
        if request.env.user.has_group(
            ...
            organisations = request.env['organisation.athletes'].sudo().search([('partner_id', '=', request.env.user.partner_id.id)]).mapped('organisation_ids').ids
            org_domain.append(('id', 'in', organisations))
        if request.httprequest.cookies.get('select_organisation') is not None:
            org_domain.append(('id', '=',
                               request.httprequest.cookies.get(
                                   'select_organisation')))
    ...
    def my_athletes_home(self, page=0, search='', **post):
        ...
        if request.env.user.has_group(
            ...
            organisations = request.env['organisation.coaches'].sudo().search(
                [('partner_id', '=', request.env.user.partner_id.id)]).mapped(
                'organisation_ids')
            org_domain.append(
                ('id', 'in', organisations.ids))
        elif request.env.user.has_group(
                'organisation.group_organisation_parents'):
            organisations = request.env['organisation.parents'].sudo().search(
                [('partner_id', '=', request.env.user.partner_id.id)]).mapped(
                'organisation_ids')
            org_domain.append(
                ('id', 'in', organisations.ids))
        else:
            raise AccessError(_("Sorry you are not allowed to this document"))
    ...
    def my_parents(self, page=0, search='', **post):
        ...
        if request.env.user.has_group('organisation.group_organisation_administrator'):
            ...
            organisations = request.env['organisation.coaches'].sudo().search([('partner_id', '=', request.env.user.partner_id.id)]).mapped('organisation_ids')
            org_domain.append(('id', 'in', organisations.ids))
        elif request.env.user.has_group(
                'organisation.group_organisation_athletes'):
            organisations = request.env[
                'organisation.athletes'].sudo().search([('partner_id', '=',
                                                         request.env.user.partner_id.id)]).mapped(
                'organisation_ids')
            org_domain.append(('id', 'in', organisations.ids))
        else:
            raise AccessError(_("Sorry you are not allowed to this records"))
        if request.httprequest.cookies.get('select_organisation') is not None:
            org_domain.append(('id', '=',
                               request.httprequest.cookies.get(
                                   'select_organisation')))
        ...
        if post:
            ...
            if request.env.user.has_group(
                ...
                org_domain.append(
                    ...
                    'organisation.group_organisation_coaches'):
                organisations = request.env[
                    'organisation.coaches'].sudo().search([('partner_id', '=',
                                                            request.env.user.partner_id.id)]).mapped(
                    'organisation_ids')
                org_domain.append(('id', 'in', organisations.ids))
            elif request.env.user.has_group(
                ...
                raise AccessError(
                    _("Sorry you are not allowed to this records"))
            if request.httprequest.cookies.get(
                    'select_organisation') is not None:
                org_domain.append(('id', '=',
                                   request.httprequest.cookies.get(
    ...
    def remove_parent(self, parent_id=None):
        if not request.env.user.has_group('organisation.group_organisation_administrator') and not request.env.user.has_group('organisation.group_organisation_coaches') or not request.env.user.has_group('organisation.group_organisation_athletes'):
            raise AccessError(_("You don't have enough access rights to run this action."))
        if parent_id:
    ...
    def my_groups(self, page=0, search='', **post):
        ...
        if request.env.user.has_group('organisation.group_organisation_administrator'):
            ...
            parents = request.env['organisation.parents'].sudo().search([('partner_id', '=', request.env.user.partner_id.id)])
            organisations = parents.mapped('organisation_ids')
            org_domain.append(('id', 'in', organisations.ids))
            athletes = parents.mapped('athlete_ids')
        ...
        if org_domain:
            ...
            if organisation:
                ...
                venues = venues.filtered(lambda x:x.organisation_ids.ids in [organisation.id])
        print(domain, "domain")
        ...
        print(venues, athletes, org_domain, "pager")
    ...
    def group_details(self, group_id=None):
        ...
        if org_domain:
            ...
            if organisation:
                ...
                venues = venues.search([('organisation_ids', 'in', [organisation.id])])
                coaches = coaches.search([('organisation_ids', 'in', [organisation.id])])
        values = {
            'group': group_id,
            'athletes': athletes,
            'coaches': coaches,
    ...
    def my_venues(self, page=0, search='', **post):
        if not request.env.user.has_group('organisation.group_organisation_administrator') and not request.env.user.has_group('organisation.group_organisation_coaches'):
            raise AccessError(_("You don't have enough access rights to run this action."))
        state_ids = list(
    ...
    def create_venue(self, **post):
        if post:
            venue_partner = request.env['res.partner'].sudo().create({
                ...
                'state_id': int(post.get('state_id')) if post.get('state_id') else None,
            ...
            values = {
                ...
                'phone': post.get('phone'),
                'responsible_user_id': int(post.get('responsible_user')) if post.get('responsible_user') else False,
                'tag_ids': [(4, tag) for tag in tag_ids],
                'organisation_ids': [(4, org) for org in organisation_ids],
            }
    ...
    def edit_chathub(self, search='', page=0, **post):
        ...
        if request.env.user.has_group('organisation.group_organisation_administrator'):
            ...
            organisations = parents.mapped('organisation_ids')
            org_domain.append(('id', 'in', organisations.ids))
            athletes = parents.mapped(
                'athlete_ids').mapped('partner_id')
            domain.append(('partner_ids', 'in', athletes.ids))
            ...
            athletes = request.env['organisation.athletes'].sudo().search(
                [('partner_id', '=', request.env.user.partner_id.id)])
            organisations = athletes.mapped('organisation_ids')
            org_domain.append(('id', 'in', organisations.ids))
            partners = athletes.mapped('partner_id')
            domain.append(('partner_ids', 'in', partners.ids))
        elif request.env.user.has_group('organisation.group_organisation_fans'):
            fans = request.env['organisation.fans'].sudo().search(
                [('partner_id', '=', request.env.user.partner_id.id)])
            org_domain.append(('id', 'in', [fans.organisation_id.id]))
            partners = fans.mapped('partner_id')
            domain.append(('partner_ids', 'in', partners.ids))
        else:
            raise AccessError(_("Not allowed to access this record"))
        if search:
            domain.append(('name', 'ilike', search))
            post["search"] = search
    ...
    def chathub_details(self, chathub_id=None):
        if not request.env.user.has_group('organisation.group_organisation_administrator') or not request.env.user.has_group('organisation.group_organisation_coaches'):
            raise AccessError(_("Sorry, You are not allowed to access this channel"))
        org_domain = []
        org_domain.append(('allowed_user_ids', 'in', [request.env.user.id]))
        if request.httprequest.cookies.get('select_organisation') is not None:
            org_domain.append(('id', '=',
