...
class ClosedSession(models.Model):
    _inherit = 'pos.config'

    session_report = fields.Boolean(
        string="Enable Session Report",
        help="Enable this option to get the reports from POS")


class ViewSession(models.Model):
    _inherit = 'pos.session'

    def get_order_summary(self, session):
        ...
        session = self.env['pos.session'].search([('id', '=', session.id)])
        ...
        if orders:
            ...
            self.env.cr.execute("""
            select sum((posl.qty * posl.price_unit) - posl.price_subtotal_incl)
            as discount,
            sum(posl.price_subtotal_incl) as gross_sale
            from pos_order join pos_order_line as posl on posl.order_id = pos_order.id
        ...
        session_summary = {
            ...
            'session_status': dict(session._fields['state'].selection).get(
                session.state),
