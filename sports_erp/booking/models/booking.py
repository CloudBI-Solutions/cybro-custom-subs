...
class Booking(models.Model):
    ...
    def notify_user(self, res):
        ...
        self.env['bus.bus']._sendone(coach.partner_id.id, 'res.partner', {
...
class BookingType(models.Model):
    ...
    product_id = fields.Many2one('product.product',
                                 domain="[('is_booking', '=', True)]")
    session_type = fields.Selection([('one2one', 'One to One'), ('team', 'Team')], default='one2one')
    price = fields.Float(string="Price")
    override_product_price = fields.Boolean('Override Product Price')

    @api.onchange('session_type', 'coach_id')
    def onchange_session_coach_id(self):
        if self.override_product_price:
            self.price = self.coach_id.price_o2o if self.session_type == 'one2one' else self.coach_id.price_team
