...
class subscription_subscription(models.Model):
    ...
    @api.model
    def _activate_frozen_subscription(self):
        subscriptions = self.env['subscription.subscription'].search([])
        for subscription in subscriptions:
            plan = subscription.sub_plan_id
            if subscription.in_frozen_stage:
                frozen_record = subscription.freeze_id
                active_stage = self.env['subscription.stage'].search(
                    [('name', 'ilike', 'Active')])
                if frozen_record.end_date == fields.Date.today():
                    subscription.write({
                        'freeze_id': False,
                        'in_frozen_stage': False,
                        'state': 'active',
            ...
            if current_rec.invoice_ids:
                for invoice_id in current_rec.invoice_ids:
                    if invoice_id.state not in ('draft', 'cancel'):
                        raise UserError(
                            _("You can't delete the record because its "
                              "invoice is create."))
    ...
    # @api.depends('start_date', 'start_immediately', 'trial_period', 'duration',
    #              'unit', 'frozen_days', 'in_frozen_stage')
    ...
    #             if current_rec.freeze_ids:
    #                 print("freeze_ids", current_rec.freeze_ids)
    #                 print("freeze_count", current_rec.freeze_count)
    #                 freeze_days = current_rec.contract_id.freeze_period
    #                 sum_freeze_days = freeze_days * current_rec.freeze_count
    #                 end_date = fields.Date.add(end_date, days=sum_freeze_days)
    ...
    contract_id = fields.Many2one('subscription.contract',  string="Contract",
                                  required=True, copy=False)
    frozen_days = fields.Integer(string="Frozen Days",
                                 related='contract_id.freeze_period')
    is_paid = fields.Boolean(string="Is Paid", compute="is_paid_subscription")
    name = fields.Char(string='Name', readonly=True)
    active = fields.Boolean(string="Active Subscription", default=True)
    ...
    state = fields.Selection([('draft', 'Draft'), ('in_progress', 'In-progress'), ('cancel', 'Cancelled'),
                              ('close', 'Finished'), ('expired', 'Expired'), ('renewed', 'Renewed'),
                              ('frozen', 'Frozen'), ('doubtful', 'Doubtful'), ('not_renewing', 'Not Renewing'),
                              ('terminated', 'Terminated')],
                             default='draft', string='State', track_visibility="always", copy=False)
    ...
    end_date = fields.Date(string="End Date", copy=False, store=True)
    currency_id = fields.Many2one('res.currency', string='Currency',
                                  default=lambda self: self.env.user.company_id.currency_id)
    so_line = fields.Many2one('sale.order.line', string='sale order line')
    freeze_ids = fields.One2many('subscription.freeze', 'subscription_id', string="Freeze Periods")
    freeze_id = fields.Many2one('subscription.freeze', store=True, copy=False)
    freeze_count = fields.Integer(string="Freeze Count", default=0)
    in_frozen_stage = fields.Boolean(string="In frozen stage", default=False)
    project_id = fields.Many2one('project.project', string="Project",
                                 copy=False)
    stage_id = fields.Many2one(
        'subscription.stage', string='Stage', index=True, tracking=True,
    ...
    #     """create methode"""
    #     result = super(subscription_subscription, self).create(vals)
    #     end_date = ""
    #     print("create\n", result.num_billing_cycle)
    #     if result.num_billing_cycle > 0:
    #         start_date = result.start_date
    #         duration = result.num_billing_cycle*result.duration
    #         if result.unit == 'day':
    #             end_date = start_date + relativedelta(days=duration)
    #         if result.unit == 'month':
    ...
    #             end_date = start_date + \
    #                 timedelta(weeks=duration)
    #         if result.freeze_ids:
    #             print("freeze_ids", result.freeze_ids)
    #             print("freeze_count", result.freeze_count)
    #             freeze_days = result.contract_id.freeze_period
    #             sum_freeze_days = freeze_days * result.freeze_count
    #             end_date = fields.Date.add(end_date, days=sum_freeze_days)
    ...
    def on_change_contract_id(self):
        for current_rec in self:
            current_rec.customer_name = current_rec.contract_id.partner_id


    # @api.onchange('customer_name')
    # def _onchange_customer(self):
    #     for rec in self:
    #         return {'domain': {'contract_id': [('employee_id.user_id', '=', rec.customer_name.id)]}}

    def _default_stage_id(self):
        """Setting default stage"""
        rec = self.env['subscription.stage'].search(
    ...
    def get_confirm_subscription(self):
        ...
        project_template = self.sub_plan_id.project_template_id
        if project_template:
            tasks = project_template.task_ids
            stages = self.env['project.task.type'].sudo().search([
                ('project_ids', 'in', project_template.id)
            ])
            project = self.env['project.project'].sudo().create({
                'name': self.customer_name.name + '-' + self.name,
                'label_tasks': project_template.label_tasks,
                'user_id': self.env.user.id,
                'partner_id': self.customer_name.id,
                'privacy_visibility': 'portal',
                'company_id': self.env.company.id,
                'date_start': self.start_date,
                'is_training': project_template.is_training,
                'is_template': False,
                'allow_timesheets': project_template.allow_timesheets,
                'allow_billable': project_template.allow_billable,
                'allowed_portal_user_ids': project_template.allowed_portal_user_ids,

            })
            for stage in stages:
                stage = self.env['project.task.type'].sudo().create({
                    'name': stage.name,
                    'mail_template_id': stage.mail_template_id.id,
                    'assign_to': stage.assign_to,
                    'assign_multi_user_ids': stage.assign_multi_user_ids.ids,
                    'sh_done': stage.sh_done,
                    'sh_cancel': stage.sh_cancel,
                    'fold': stage.fold,
                    'is_closed': stage.is_closed,
                    'task_type_selection': stage.task_type_selection,
            ...
            for task in tasks:
                ...
                self.env['project.task'].sudo().create({
                    ...
                    'stage_id': stage.id,
                    'org_stage_id': stage.id,
                    'sequence': task.sequence,
                    'task_type_selection': task.task_type_selection,
                    'assign_type_selection': task.assign_type_selection,
                    'assign_athlete_ids': task.assign_athlete_ids,
                    'assign_group_ids': task.assign_group_ids,
                    'assign_coach_ids': task.assign_coach_ids,
                    'task_checkbox': task.task_checkbox,
        ...
        #     return {'warning': {
        #         'title': _("Warning"),
        #         'message': _('There is no linked project')}}
        active_stage = self.env['subscription.stage'].search(
            [('name', 'ilike', 'Active')])
        ...
        elif self.source == 'so' or self.source == 'website':
            if self.so_origin.invoice_count != 0:
                self.action_invoice_create()
            else:
                invoice = self.so_origin.invoice_ids
                if self.invoice_ids:
                    # invoice =
                    self.invoice_ids = [(4,  self.so_origin.invoice_ids.ids)]
                else:
                    self.invoice_ids = [self.so_origin.invoice_ids.ids]
                start_date = datetime(year=self.start_date.year,
                                      month=self.start_date.month,
                                      day=self.start_date.day, minute=0, hour=0,
                                      second=0) if self.source == 'manual' else self.start_date + relativedelta(
                    days=1)
                if not isinstance(start_date, datetime):
                    start_date = datetime(*start_date.timetuple()[:6])
                if self.num_billing_cycle != self.invoice_count:
                    if self.num_billing_cycle > 0:
                        end_date = datetime(year=self.end_date.year,
                                            month=self.end_date.month,
                                            day=self.end_date.day, minute=0,
                                            hour=0, second=0)
                        date_intervals = self.cal_date_period(start_date,
                                                              end_date,
                                                              self.num_billing_cycle)
                        self.next_payment_date = datetime.strptime(
                            date_intervals[self.invoice_count - 1],
                            "%d/%m/%Y %H:%M:%S")
                    else:
    ...
    #             'company_id': self.env.company.id,
    #         })
    #         tasks = self.env['subscription.tasks'].search([('sub_plan_id', '=', self.sub_plan_id.id)])
    #         for task in tasks:
    ...
    def get_frozen_subscription(self):
        contract = self.contract_id
        if self.freeze_count >= contract.allowed_freeze_count:
            raise ValidationError('Reached Allowed Freeze Limit')
        else:
            frozen_stage = self.env['subscription.stage'].search(
                [('name', 'ilike', 'Frozen')])
            obj_freeze = self.env['subscription.freeze'].sudo().create({
            ...
            self.write({
                ...
                'freeze_count': self.freeze_count + 1,
                'price': contract.freeze_price,
            })
            if self.end_date:
                self.write({
                    'end_date': fields.Date.add(self.end_date,
                                                days=contract.freeze_period),
                })
            if self.next_payment_date:
                self.write({
                    'next_payment_date': fields.Date.add(
                        self.next_payment_date, days=contract.freeze_period),
                })
            self.action_invoice_create()
        # for current_rec in self:
        #     current_rec.state = 'frozen'
        #     self.stage_id = frozen_stage.id

    def get_doubtful_subscription(self):
        doubtful_stage = self.env['subscription.stage'].search(
            [('name', 'ilike', 'Doubtful')])
        for current_rec in self:
            current_rec.state = 'doubtful'
            self.stage_id = doubtful_stage.id

    def get_not_renewing_subscription(self):
        not_renewing_stage = self.env['subscription.stage'].search(
            [('name', 'ilike', 'Not Renewing')])
        for current_rec in self:
            current_rec.state = 'not_renewing'
            self.stage_id = not_renewing_stage.id

    def get_terminated_subscription(self):
        terminated_stage = self.env['subscription.stage'].search(
            [('name', 'ilike', 'Terminated')])
        for current_rec in self:
            current_rec.state = 'terminated'
            self.stage_id = terminated_stage.id

    def action_archive_subscription(self):
        self.write({
            'active': False,
        })

    def action_unarchive_subscription(self):
        self.write({
            'active': True,
        })
    ...
    def action_invoice_create(self, grouped=False, final=False):
        ...
        for subscription in self:
            ...
            if subscription.num_billing_cycle == subscription.invoice_count or subscription.num_billing_cycle < subscription.invoice_count and subscription.num_billing_cycle != -1:
                ...
                return True
            group_key = subscription.id if grouped else (subscription.customer_name.id, subscription.product_id.sudo().currency_id.id)
    ...
    def _prepare_invoice_line(self, qty):
        ...
        product = self.product_id.with_context(
            lang=self.customer_name.sudo().lang,
            partner=self.customer_name.sudo().id,
        ...
        res = {
            ...
            'tax_ids': [(6, 0, self.sudo().tax_id.ids)],
        }
        return res
            ...
            if current_rec.invoice_ids:
                for invoice_id in current_rec.invoice_ids:
                    if invoice_id.amount_residual_signed > 0.0:
                        ...
                        self.env['account.payment'].sudo().create({'journal_id':invoice_id.journal_id.id,'amount':invoice_id.amount_total,'communication':invoice_id.name,'payment_type':'inbound','payment_method_id':self.env['account.payment.method'].search([('payment_type', '=', 'inbound')], limit=1).id,'partner_type':'customer','partner_id':invoice_id.partner_id.id,}).post()
    ...
    # @api.model
    # def create(self, vals):
    #     subscription = super(subscription_subscription, self).create(vals)
    #     for current_rec in self:
    #         print('ggg')
    #         project = self.env['project.project'].create({
    #             'name': current_rec.name,
    #             'label_tasks': 'Sessions',
    #             'user_id': current_rec.env.user.id,
    #             'partner_id': current_rec.customer_name.id,
    #             'privacy_visibility': 'portal',
    #             'company_id': current_rec.company_id.id,
    #         })
    #         parent_task = self.env['project.task'].create({
    #             'project_id': project.id,
    #             'name': 'Milestone'
    #         })
    #         sessions = self.product_id.sessions
