...
class ResConfigSettings(models.TransientModel):
    ...
    allow_cancel = fields.Boolean(string="Cancel", default=False)
    allow_freeze = fields.Boolean(string="Freeze", default=False)
    ...
    def get_values(self):
        ...
        res.update({
            'journal_id': IrDefault.get(
                'res.config.settings', 'journal_id'),
            'invoice_generated': IrDefault.get(
                'res.config.settings', 'invoice_generated'),
            'invoice_email': IrDefault.get(
                'res.config.settings', 'invoice_email'),
            'trial_period_setting': IrDefault.get(
                'res.config.settings', 'trial_period_setting'),
            'allow_cancel': IrDefault.get(
                'res.config.settings', 'allow_cancel'),
            'allow_freeze': IrDefault.get(
                'res.config.settings', 'allow_freeze'),
        })
        return res
    ...
    def set_values(self):
        ...
        IrDefault = self.env['ir.default'].sudo()
        IrDefault.set('res.config.settings', 'journal_id',
                      self.journal_id.id)
        IrDefault.set('res.config.settings', 'invoice_generated',
                      self.invoice_generated)
        IrDefault.set('res.config.settings', 'invoice_email',
                      self.invoice_email)
        IrDefault.set('res.config.settings', 'trial_period_setting',
                      self.trial_period_setting)
        IrDefault.set('res.config.settings', 'allow_cancel',
                      self.allow_cancel)
        IrDefault.set('res.config.settings', 'allow_freeze',
                      self.allow_freeze)
