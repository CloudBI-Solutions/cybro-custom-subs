...
class BadmintooVideoController(http.Controller):
    ...
    def upload_rcfz_videos(self, **post):
        ...
        if post.get('rcbz_co_fnd_video'):
            print('herree')
            attachment = request.env['ir.attachment'].sudo().search([
                ('res_model', '=', 'badminto.assessment'),
                ('res_id', '=', assessment.id),
                ('badminto_field_name', '=', 'rcbz_co_fnd_video')
            ])
            if attachment:
                attachment.unlink()
            baseto64 = \
                str(base64.b64encode(
                    post.get('rcbz_co_fnd_video').read())).split(
                    "b'")[1]
            file_data = 'data:video/mp4;base64,' + baseto64
            request.env['ir.attachment'].sudo().create({
                'res_model': 'badminto.assessment',
                ...
                'type': 'binary',
                'badminto_field_name': 'rcbz_co_fnd_video',
                'name': post.get('rcbz_co_fnd_video').filename,
                'badminto_video_file_data': file_data,
                'datas': base64.b64encode(
                    post.get('rcbz_co_fnd_video').read())
            })
