...
class ResConfigSettings(models.TransientModel):
    ...
    _pwa_icon_url_base = "/paceflow_pwa/icon"
    ...
    pwa_name = fields.Char(
        string='PWA Name',
        default='Pace-Flow',
        help='Pace-Flow Progressive Web Application Name')

    pwa_short_name = fields.Char(
        string='PWA Short Name',
        default='Pace-Flow',
        help='Pace-Flow Progressive Web Application Short Name')

    pwa_description = fields.Char(
        String='PWA Description',
        default='Pace-Flow PWA',
        help='Pace-Flow Progressive Web Application Description')
    ...
    def _get_default_icon(self):
        image_path = modules.get_module_resource('paceflow_pwa',
                                                 'static/src/img',
                                                 'paceflow-logo_512.png')
    ...
    def set_values(self):
        ...
        set_values.set_param('paceflow_pwa.activate_pwa',
                             self.activate_pwa)
        set_values.set_param("paceflow_pwa.pwa_name",
                             self.pwa_name)
        set_values.set_param("paceflow_pwa.pwa_short_name",
                             self.pwa_short_name)
        set_values.set_param('paceflow_pwa.pwa_description',
                             self.pwa_description)
        set_values.set_param("paceflow_pwa.pwa_background_color",
                             self.pwa_background_color)
        set_values.set_param("paceflow_pwa.pwa_theme_color",
    ...
    def get_values(self):
        ...
        activate_pwa = get_values.get_param('paceflow_pwa.activate_pwa')
        pwa_name = get_values.get_param(
            "paceflow_pwa.pwa_name")
        pwa_short_name = get_values.get_param(
            "paceflow_pwa.pwa_short_name")
        pwa_description = get_values.get_param(
            "paceflow_pwa.pwa_description")
        pwa_background_color = get_values.get_param(
            "paceflow_pwa.pwa_background_color")
        pwa_theme_color = get_values.get_param(
            "paceflow_pwa.pwa_theme_color")
        ...
        if activate_pwa:
            res.update(
                ...
                pwa_icon=pwa_icon_ir_attachment.datas if pwa_icon_ir_attachment else False
            )
        return res
