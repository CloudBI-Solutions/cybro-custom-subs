odoo.define('ks_website_dashboard_ninja.ks_website_dashboard', function(require) {
    ...
    publicWidget.registry.snippet_dashboard_home_page = publicWidget.Widget.extend({
        ...
        init: function(parent, state, params) {
            ...
            self.form_template = 'ks_dashboard_ninja_template_view';
            self.gridstackConfig = {};
            var l10n = _t.database.parameters;
//            self.ks_date_filter_selections = {
...
//            };
            var ks_date_filter_selection = {}
            ajax.jsonRpc('/fetch/date_filter_selection/data', 'call', {
                model: 'ks_dashboard_ninja.item',
                method: 'ks_fetch_date_filter_selection',
                args: [],
                kwargs: {
                   'type': 'all',
                }
                ...
                if (result){
                    result.forEach(function(date_filter){
                        ks_date_filter_selection[date_filter[0]] = date_filter[1];
                   });
                   self.ks_date_filter_selections = ks_date_filter_selection;
                }

            });
            self.ks_date_filter_selection_order = ['l_day', 't_week', 't_month', 't_quarter', 't_year', 'n_day',
            ...
            self.date_format = time.strftime_to_moment_format(_t.database.parameters.date_format)
            self.date_format = this.date_format.replace(/\bYY\b/g, "YYYY");
            self.datetime_format = time.strftime_to_moment_format((l10n.date_format + ' ' + l10n.time_format));
            ...
            self.grid = false;
            self.ksChartColorOptions = ['default', 'cool', 'warm', 'neon'];
...
//                disableOneColumnMode : true,
        ...
        start: function(dashboard_id, data_selection, ks_self_received) {
            console.log("TTTTTTTTTTTTTTTTTTTTT",dashboard_id,data_selection,ks_self_received)
            var ks_self = this;
