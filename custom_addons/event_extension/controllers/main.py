...
class WebsiteEventControllerExtended(http.Controller):
    @http.route(['''/event/<model("event.event"):event>/registration/new'''], type='http', auth="public",
                methods=['POST'], website=True)
    def registration_new(self, **post):
        print(post)
        ...
        for i in range(0, int(limit)):
            for j in range(0, len(ticket_ids[i])):
                if ticket_ids[i][j] != 0:
                    ticket_id = request.env['event.event.ticket'].sudo().search([('id', '=', ticket_ids[i][j])])
                    print("test")
                    cart_values = order.with_context(event_ticket_id=ticket_id.id, fixed_price=True)._cart_update(
                    ...
                    request.env['event.registration'].sudo().create({
                                    ...
                                    'emergency_contact_number_1': str(post.get('emergency_no_1')),
                                    'emergency_contact_number_2': str(post.get('emergency_no_2')),
                                    'signature': imgdata,
