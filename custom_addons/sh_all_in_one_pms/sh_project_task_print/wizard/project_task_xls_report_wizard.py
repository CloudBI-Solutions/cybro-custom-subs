...
class ProjectTask(models.Model):
    ...
    def action_project_task_xls_entry(self):
        action = self.env['ir.actions.act_window']._for_xml_id(
            'sh_all_in_one_pms.sh_project_task_details_report_wizard_form_action')
...
class ShProjectTaskDetailsReportWizard(models.TransientModel):
    ...
    def print_task_xls_report(self):
        ...
        for task in Tasks:
            ...
            final_value = {}
            users = []
            heading = task.name + ' ( ' + task.stage_id.name + ' )'
            progress = str(task.progress) + ' %'
            subtask_planned_hours = str(
                task.subtask_planned_hours) + ' planned hours'
            ...
            final_value['email_from'] = task.email_from
            final_value['date_deadline'] = task.date_deadline
            if task.description:
                final_value['description'] = handle.handle(task.description)
            final_value['planned_hours'] = str(task.planned_hours)
            final_value['progress'] = task.progress
            final_value['subtask_planned_hours'] = subtask_planned_hours
            ...
            if final_value['email_from']:
                worksheet.write_merge(
                    7, 8, 1, 1, final_value['email_from'], format2)
            else:
            ...
            if task.planned_hours:
                ...
                if final_value['planned_hours']:
                    worksheet.write_merge(
                        row, row+1, 1, 1, final_value['planned_hours'], format2)
                else:
                ...
                if final_value['subtask_planned_hours']:
                    worksheet.write_merge(
                        row, row+1, 1, 1, final_value['subtask_planned_hours'], format2)
                else:
                    worksheet.write_merge(row, row+1, 1, 1, '', format2)
                row += 3
