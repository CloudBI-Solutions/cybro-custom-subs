...
class ProductProduct(models.Model):
    ...
    activate_subscription = fields.Boolean(string="Is Subscription Type",
                                           default=False)
    ...
    is_subscription_addon = fields.Boolean('Is Subscription Addon',
                                           default=False)
    freeze_months = fields.Integer('Months to freeze', default='1')
    sessions = fields.Integer('No of Sessions', default='1', readonly=False)
    ...
    def on_change_activate_subscription(self):
        for current_rec in self:
            if current_rec.activate_subscription:
                ...
                current_rec.type = 'service'
                current_rec.is_subscription_addon = False
            else:
                current_rec.invoice_policy = 'order'

    @api.onchange('is_subscription_addon')
    def on_change_is_subscription_addon(self):
        for current_rec in self:
            if current_rec.is_subscription_addon:
                current_rec.invoice_policy = 'delivery'
                current_rec.type = 'service'
                current_rec.activate_subscription = False
            else:
                current_rec.invoice_policy = 'order'
    ...
    def write(self, vals):
        if vals.get('activate_subscription'):
            vals['type'] = 'service'
        if vals.get('is_subscription_addon'):
            vals['type'] = 'service'
...
class ProductTemplate(models.Model):
    ...
    subscription_contract_id = fields.Many2one(
        comodel_name="subscription.contract",
        related="product_variant_ids.subscription_contract_id",
        string="Subscription Contract", readonly=False)
    is_subscription_addon = fields.Boolean(related='product_variant_ids.is_subscription_addon',
                                           string='Is Subscription Addon',
                                           readonly=False)
    freeze_months = fields.Integer(related='product_variant_ids.freeze_months',
                                   string='Months to freeze',
                                   default='1', readonly=False)

    sessions = fields.Integer(related='product_variant_ids.sessions', string='No of Sessions', default='1', readonly=False)
    ...
    def create(self, vals):
        if vals.get('activate_subscription'):
            vals['type'] = 'service'
        elif vals.get('is_subscription_addon'):
            vals['type'] = 'service'
        ...
        if vals.get('is_subscription_addon'):
            related_vals['is_subscription_addon'] = vals[
                'is_subscription_addon']

        if vals.get('freeze_months'):
            related_vals['freeze_months'] = vals[
                'freeze_months']

        if vals.get('sessions'):
            related_vals['sessions'] = vals[
                'sessions']
        ...
        for invoice_id in self:
            if invoice_id.amount_residual_signed > 0.0:
                ...
                if invoice_generated=='paid':
                    ...
                    self.env['account.payment'].sudo().create({'journal_id':invoice_id.journal_id.id, 'amount':invoice_id.amount_total,'payment_type':'inbound','payment_method_id':self.env['account.payment.method'].search([('payment_type', '=', 'inbound')], limit=1).id,'partner_type':'customer','partner_id':invoice_id.partner_id.id,}).action_post()
                    _logger.info("===================================")
